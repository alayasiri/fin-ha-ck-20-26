...
def render(protocol_name: str, scores: dict, tvl_data: dict, anomalies: dict):
    ...
    if tvl_series:
        ...
        for incident in HISTORICAL_INCIDENTS.get(protocol_name, []):
            try:
                ...
                continue
            line_color = "#f85149" if incident["severity"] == "major" else "#f0883e"
            fig_tvl.add_vline(
    ...
    if compare_with:
        ...
        fig_cmp = go.Figure()
        colors_cmp = ["#58a6ff", "#3fb950", "#f0883e", "#d2a8ff"]
        ...
        fig_cmp.update_layout(
            ...
            plot_bgcolor="#161b22",
            xaxis=dict(showgrid=False, color="#8b949e"),
            yaxis=dict(title="% of Peak TVL", gridcolor="#21262d", color="#8b949e"),
            legend=dict(bgcolor="#161b22", bordercolor="#30363d", font=dict(color="#e6edf3")),
            hovermode="x unified",
        )
